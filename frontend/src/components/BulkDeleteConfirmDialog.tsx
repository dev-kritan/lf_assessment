        ...
        <div className="my-4">
          <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2 flex items-center justify-between">
            <span className="flex items-center gap-1.5">
              {isCreated ? <Calendar className="w-3.5 h-3.5 text-indigo-500" /> : <Users className="w-3.5 h-3.5 text-emerald-500" />}
              Selected {isCreated ? 'Events' : 'RSVPs'} ({count}):
            </span>
            <span className="text-[11px] text-slate-500">
              {count > 4 ? `Showing 4 of ${count}` : ''}
            </span>
          </div>

          <div className="max-h-48 overflow-y-auto space-y-2 rounded-2xl bg-slate-50 dark:bg-slate-800/60 p-3 border border-slate-200/80 dark:border-slate-800">
            {selectedEvents.slice(0, 4).map((evt) => (
              <div
                key={evt.id}
                className="flex items-center justify-between gap-3 text-xs py-1.5 px-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80"
              >
                ...
                {evt.userRsvp && (
                  <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 capitalize flex-shrink-0">
                    {evt.userRsvp}
                  </span>
                )}
              </div>
            ...
            {count > 4 && (
              <div className="text-center text-[11px] font-medium text-slate-500 dark:text-slate-400 py-1">
                + and {count - 4} more event{count - 4 === 1 ? '' : 's'}
              </div>
            )}
          </div>
        </div>
