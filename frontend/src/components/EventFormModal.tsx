...
export const EventFormModal: React.FC<EventFormModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (eventToEdit) {
      ...
      setIsTruePrivate(
        eventToEdit.eventType === "private"
          ? Boolean(eventToEdit.isTruePrivate)
          : false,
      );
      try {
        setStartTime(
          format(parseISO(eventToEdit.startTime), "yyyy-MM-dd'T'HH:mm"),
        );
        if (eventToEdit.endTime) {
          setEndTime(
            format(parseISO(eventToEdit.endTime), "yyyy-MM-dd'T'HH:mm"),
          );
        } else {
          setEndTime("");
        }
  ...
  const validateFieldOnChange = (
    ...
    const currentData = {
      ...
      capacity:
        (updatedValues.capacity ?? capacity)
          ? Number(updatedValues.capacity ?? capacity)
          : undefined,
      bannerUrl:
        (updatedValues.bannerUrl ?? bannerUrl)
          ? (updatedValues.bannerUrl ?? bannerUrl).trim()
          : undefined,
  ...
  const handleAddNewTag = async () => {
    ...
    if (existingTag) {
      setSelectedTagIds((prev) => {
        const updated = prev.includes(existingTag.id)
          ? prev
          : [...prev, existingTag.id];
        validateFieldOnChange("tagIds", { tagIds: updated });
        return updated;
  ...
  return (
    <div
      className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-2.5 sm:p-6 animate-fade-in"
      onClick={onClose}
    >
      <div
        className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-auto sm:my-8 max-h-[92dvh] sm:max-h-[85vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-4 py-3.5 sm:px-6 sm:py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0">
        ...
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
              <button
                ...
                onClick={() => {
                  setEventType("public");
                  setIsTruePrivate(false);
                  validateFieldOnChange("eventType", {
                    eventType: "public",
                    isTruePrivate: false,
                  });
                }}
                ...
                <button
                  ...
                  onClick={() => {
                    if (eventType === "private") {
                      ...
                      validateFieldOnChange("isTruePrivate", {
                        isTruePrivate: nextVal,
                      });
                    }
                  }}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3 max-h-36 overflow-y-auto p-1.5 border border-slate-100 dark:border-slate-800/60 rounded-2xl bg-slate-50/50 dark:bg-slate-950/40">
              {availableTags.length === 0 ? (
                <p className="text-xs text-slate-400 dark:text-slate-500 py-1 px-2">
                ...
                availableTags.map((tag) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-semibold transition-all ${
                      selectedTagIds.includes(tag.id)
                        ? "ring-2 ring-indigo-400/40 shadow-sm"
          ...
          <div className="pt-3 sm:pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-2 sm:gap-3 sticky bottom-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md pb-1 sm:pb-0 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-3.5 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              className="px-5 sm:px-6 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-semibold text-xs sm:text-sm shadow-md shadow-indigo-500/20 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
