...
describe('Assessment Bonus SQL Queries (Q1, Q2, Q4)', () => {
    it('should fetch raw bonus tables', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/v1/bonus/data');
    ...
    it('should execute Q2 and return timeline with previous and next designations', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/v1/bonus/q2');
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        expect(res.body.data.rows.length).toBe(24);
        const rows = res.body.data.rows;
        const aliceRows = rows.filter((r) => r.emp_id === 'EMP001');
    ...
    it('should execute Q4 and return active designation at project allocation start', async () => {
        const res = await (0, supertest_1.default)(app).get('/api/v1/bonus/q4');
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        ...
        const a001 = rows.find((r) => r.allocation_id === 'A001');
        ...
        const a002 = rows.find((r) => r.allocation_id === 'A002');
        ...
        const a003 = rows.find((r) => r.allocation_id === 'A003');
