...
describe('Email Verification & Nodemailer Integration', () => {
    ...
    it('should register user with SHA-256 hashed token and 24h expiration', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/v1/auth/register').send(newUser);
        ...
        const dbUser = await (0, knex_1.default)(constants_1.DB_TABLES.USERS).where({ id: userId }).first();
        ...
        expect(email_service_1.EmailService.lastSentEmail).toBeDefined();
        expect(email_service_1.EmailService.lastSentEmail.to).toBe(newUser.email.toLowerCase());
        expect(email_service_1.EmailService.lastSentEmail.verificationUrl).toContain(`token=${rawToken}`);
        expect(email_service_1.EmailService.lastSentEmail.verificationUrl).toContain(`uid=${userId}`);
    });
    ...
    it('should reject expired verification token with VERIFICATION_TOKEN_EXPIRED', async () => {
        ...
        await (0, knex_1.default)(constants_1.DB_TABLES.USERS)
            .where({ id: userId })
            .update({
    ...
    it('should resend verification email with a new 24h token', async () => {
        const res = await (0, supertest_1.default)(app).post('/api/v1/auth/resend-verification').send({
            email: newUser.email,
        });
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        ...
        expect(email_service_1.EmailService.lastSentEmail).toBeDefined();
        expect(email_service_1.EmailService.lastSentEmail.to).toBe(newUser.email.toLowerCase());
        rawToken = email_service_1.EmailService.lastSentEmail.rawToken || email_service_1.EmailService.lastSentEmail.verificationUrl.split('token=')[1].split('&')[0];
    });
    ...
    it('should enforce rate limiting on resend verification endpoint when requested with test flag', async () => {
        ...
        const blockedRes = await (0, supertest_1.default)(app)
            .post('/api/v1/auth/resend-verification')
            .set('x-test-rate-limit', 'true')
            .send({ email: testEmail });
    ...
    describe('Unverified User Permissions & Access Control', () => {
        let unverifiedToken = '';
        let verifiedToken = '';
        let truePrivateEventId;
        let standardPrivateEventId;
        let publicEventId;
        beforeAll(async () => {
            // Login as Carol (unverified user in seeds)
            const carolRes = await (0, supertest_1.default)(app).post('/api/v1/auth/login').send({
            ...
            const aliceRes = await (0, supertest_1.default)(app).post('/api/v1/auth/login').send({
            ...
            const pubRes = await (0, supertest_1.default)(app)
                .post('/api/v1/events')
            ...
            const stdRes = await (0, supertest_1.default)(app)
                .post('/api/v1/events')
                .set('Authorization', `Bearer ${verifiedToken}`)
                .send({
            ...
            const truePrivRes = await (0, supertest_1.default)(app)
                .post('/api/v1/events')
                .set('Authorization', `Bearer ${verifiedToken}`)
                .send({
        ...
        it('unverified user can view public events and standard private events, but NOT true private events in list', async () => {
            ...
            expect(res.status).toBe(200);
            const eventIds = res.body.data.map((e) => e.id);
        ...
        it('unverified user receives masked/protected fields on standard private events, while public events remain fully visible', async () => {
            ...
            const verifiedRes = await (0, supertest_1.default)(app)
                .get(`/api/v1/events/${standardPrivateEventId}`)
                .set('Authorization', `Bearer ${verifiedToken}`);
        ...
        it('unverified user can request verification email from authenticated profile endpoint', async () => {
            ...
            expect(res.body.data.verificationToken).toBeDefined();
            expect(email_service_1.EmailService.lastSentEmail.to).toBe('carol@example.com');
        });
    });
});
