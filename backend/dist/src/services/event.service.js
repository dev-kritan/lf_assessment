...
class EventService {
    static async getEvents(params, currentUserId, isVerified = false) {
        ...
        const totalResult = await knex_1.default.from(baseQuery.clone().clearSelect().select('events.id').as('filtered_events')).count('* as count').first();
        const total = Number(totalResult?.count || 0);
        // Sorting
        const sortOrder = (params.sort_order || (params.timeframe === 'past' ? 'desc' : 'asc')).toLowerCase();
        if (params.sort_by === 'popularity') {
            // Sort by RSVP Yes count
            baseQuery
                .select(knex_1.default.raw('(SELECT COUNT(*) FROM rsvps WHERE rsvps.event_id = events.id AND rsvps.status = "yes") as rsvp_yes_count'))
        ...
        if (eventIds.length > 0) {
            ...
            rsvpCounts.forEach((r) => {
                ...
                if (r.status === 'yes')
                    rsvpStatsMap[eId].yes = count;
                if (r.status === 'maybe')
                    rsvpStatsMap[eId].maybe = count;
                if (r.status === 'no')
                    rsvpStatsMap[eId].no = count;
    ...
    static async createEvent(data, creatorId) {
        ...
        return await knex_1.default.transaction(async (trx) => {
            ...
            const eventId = typeof eventIdRaw === 'object' ? eventIdRaw.id || 1 : eventIdRaw;
