...
class RsvpService {
    static async setRsvp(eventId, userId, status) {
        const event = await (0, knex_1.default)(constants_1.DB_TABLES.EVENTS).where({ id: eventId }).first();
        if (!event) {
        ...
        if (event.capacity && status === 'yes') {
            const existingRsvp = await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS).where({ event_id: eventId, user_id: userId }).first();
            if (!existingRsvp || existingRsvp.status !== 'yes') {
                const [yesCountResult] = await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS)
                ...
                if (yesCount >= event.capacity) {
                    ...
                    error.code = constants_1.ERROR_CODES.CAPACITY_REACHED;
                    throw error;
                }
            }
        }
        const existing = await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS).where({ event_id: eventId, user_id: userId }).first();
        if (existing) {
            ...
            await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS).insert({
                event_id: eventId,
                user_id: userId,
                status,
            });
    ...
    static async getAttendees(eventId) {
        const attendees = await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS)
    ...
    static async getUserRsvps(userId) {
        const rsvps = await (0, knex_1.default)(constants_1.DB_TABLES.RSVPS)
            .join(constants_1.DB_TABLES.EVENTS, 'rsvps.event_id', 'events.id')
            .leftJoin(`${constants_1.DB_TABLES.USERS} as creator`, 'events.creator_id', 'creator.id')
