...
export async function seed(knex: Knex): Promise<void> {
  ...
  const u1 = typeof user1Id === 'object' ? (user1Id as any).id || 1 : (user1Id || 1);
  const u2 = typeof user2Id === 'object' ? (user2Id as any).id || 2 : (user2Id || 2);
  const u3 = typeof user3Id === 'object' ? (user3Id as any).id || 3 : (user3Id || 3);
  ...
  if (summitId) {
    rsvpsToInsert.push(
      { event_id: summitId, user_id: u1, status: 'yes' },
      { event_id: summitId, user_id: u2, status: 'maybe' },
      { event_id: summitId, user_id: u3, status: 'yes' }
    );
  }
  if (workshopId) {
    rsvpsToInsert.push(
      { event_id: workshopId, user_id: u2, status: 'yes' },
      { event_id: workshopId, user_id: u3, status: 'maybe' }
    );
  }
  if (birthdayId) {
    rsvpsToInsert.push(
