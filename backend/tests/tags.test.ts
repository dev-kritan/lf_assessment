...
describe('Tags Endpoints (Usage, Update, Delete & Cascade)', () => {
  ...
  afterAll(async () => {
    ...
    if (testTagId) {
      await db('tags').where({ id: testTagId }).delete();
    }
  });

  it('GET /api/v1/tags/:id/usage should return associated events and event count', async () => {
    const res = await request(app).get(`/api/v1/tags/${testTagId}/usage`);
    ...
    expect(res.body.data.associatedEvents.some((e: any) => e.id === testEventId)).toBe(true);
  });

  it('PUT /api/v1/tags/:id should reject unauthenticated request', async () => {
    const res = await request(app).put(`/api/v1/tags/${testTagId}`).send({
      name: 'UpdatedTagName',
    });
  ...
  it('PUT /api/v1/tags/:id should update tag name and color', async () => {
    ...
    // Verify in database
    const inDb = await db('tags').where({ id: testTagId }).first();
  ...
  it('PUT /api/v1/tags/:id should reject duplicate tag name', async () => {
    ...
    // Clean up tag2
    await db('tags').where({ id: tag2Id }).delete();
  });

  it('DELETE /api/v1/tags/:id should reject unauthenticated request', async () => {
    const res = await request(app).delete(`/api/v1/tags/${testTagId}`);
    expect(res.status).toBe(401);
  });
  ...
  it('DELETE /api/v1/tags/:id should delete tag and cascade delete from event_tags', async () => {
    ...
    const beforeEventTag = await db('event_tags')
    ...
    const afterEventTag = await db('event_tags')
      .where({ tag_id: testTagId })
      .first();
    expect(afterEventTag).toBeUndefined();

    // Verify event still exists
    const eventStillExists = await db('events').where({ id: testEventId }).first();
